...
from api.core.responses import redirect
from api.core.context import request
from api.core.context import session
from api.core.context import url_for
...
# NOTE(fastapi-port): route names keep the legacy flask endpoint names
# (``oauth2.login`` / ``oauth2.callback`` / ``oauth2.logout``) so that
# ``url_for('oauth2.callback', ...)`` etc. keep resolving.
@router.get('/api/{auth_type}/login', name='oauth2.login')
def login(auth_type: str):
...
@router.get('/api/{auth_type}/callback', name='oauth2.callback')
def callback(auth_type: str):
    auth_type = auth_type.upper()
...
@router.get('/api/{auth_type}/logout', name='oauth2.logout')
def logout(auth_type: str):
