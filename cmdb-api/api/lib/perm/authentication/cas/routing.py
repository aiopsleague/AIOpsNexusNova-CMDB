...
# NOTE(fastapi-port): the primary route carries the legacy flask endpoint name
# ``cas.login`` so that ``url_for('cas.login')`` keeps resolving.
@router.get('/api/sso/login')
@router.get('/api/cas/login', name='cas.login')
def login():
    """
...
@router.get('/api/sso/logout')
@router.get('/api/cas/logout', name='cas.logout')
def logout():
    """
