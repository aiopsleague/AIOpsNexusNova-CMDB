...
function init(open: boolean, isInitOne = true) {
  // isInitOne: when the parsed expression is empty, whether the rule list gets a default entry.
  ...
  if (open && exp) {
    const expArray = exp.split(',').map((item) => {
      ...
      return { id: uuidv4(), type, property, exp: expValue, value, min, max, compareType }
    })
