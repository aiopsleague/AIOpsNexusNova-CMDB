...
const subnetCITypeId = computed(() => props.subnetCIType?.id || null)
...
async function getAttributeList() {
  await getCITypeAttributesById(subnetCITypeId.value as number).then((res) => {
...
async function getPreferenceAttrList() {
  const subscribed = await getSubscribeAttributes(subnetCITypeId.value as number)
...
async function getTableData() {
  try {
    ...
    const res = await searchCI({
      q: `_type:${subnetCITypeId.value}${exp ? `,${exp}` : ''}${fuzzySearch ? `,*${fuzzySearch}*` : ''}`,
...
function copyExpression() {
  ...
  const regQ = /(?<=q=).+(?=&)|(?<=q=).+$/g
  const exp = expression.match(regQ) ? expression.match(regQ)[0] : null
  const text = `q=_type:${subnetCITypeId.value}${exp ? `,${exp}` : ''}${fuzzySearch ? `,*${fuzzySearch}*` : ''}`
  navigator.clipboard
    .writeText(text)
    .then(() => {
...
function openBatchDownload() {
  batchDownloadRef.value?.open({
    ...
    ciTypeName: t('cmdb.ipam.subnetList') || '',
  })
}
...
<template>
  <div ref="wrapRef">
    ...
    <CIDetailDrawer ref="detailRef" :type-id="subnetCITypeId" />

    <CreateInstanceForm
      ref="createRef"
      :type-id-from-prop="subnetCITypeId"
      @submit="batchUpdate"
    />
  </div>
</template>
