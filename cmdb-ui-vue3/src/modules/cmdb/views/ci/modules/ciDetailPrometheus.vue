...
const emit = defineEmits<{
  (e: 'connectionStatusChange', statuses: any[]): void
}>()

const SEVERITY_LIST = [
  { key: 'disaster', icon: CloseCircleOutlined },
  { key: 'emergency', icon: CloseCircleOutlined },
  { key: 'critical', icon: CloseCircleOutlined },
  { key: 'important', icon: ExclamationCircleOutlined },
  { key: 'warning', icon: ExclamationCircleOutlined },
...
const connectionStatuses = ref<any[]>([])
...
const severityList = computed(() =>
  SEVERITY_LIST.map((s) => ({
    ...s,
    label: severityStatusText(s.key),
  }))
)

const unhealthyConnections = computed(() => connectionStatuses.value.filter((s) => !s.ok))

const healthyCount = computed(() => connectionStatuses.value.filter((s) => s.ok).length)

const allConnectionsUnhealthy = computed(
  () =>
    connectionStatuses.value.length > 0 &&
    unhealthyConnections.value.length === connectionStatuses.value.length
)

const emptyDescription = computed(() => {
  if (errorMsg.value) return errorMsg.value
  if (!configured.value) return t('cmdb.ci.alertNoConfig')
  if (unhealthyConnections.value.length > 0) return t('cmdb.ci.alertConnectionFailed')
  return t('cmdb.ci.alertNoData')
})

const filteredAlerts = computed(() => {
  if (filterSeverity.value === null) return alerts.value
  return alerts.value.filter((a) => _severity(a) === filterSeverity.value)
})
...
const columns = computed(() => {
  ...
  const cols: any[] = [
    { title: t('cmdb.ci.alertSeverity'), dataIndex: 'labels.severity', key: 'severity', width: 110 },
  ]
  // Dynamic columns from display_columns config.
  // Backend flattens values to _d_<safe_key> where dots are replaced with "__".
  ;(displayColumns.value || []).forEach((col: any) => {
  ...
  cols.push(
    { title: t('cmdb.ci.alertActiveAt'), dataIndex: 'activeAt', key: 'activeAt', width: 180 },
    { title: t('cmdb.ci.alertDuration'), dataIndex: 'activeAt', key: 'duration', width: 130 }
  )
  return cols
})

function _severity(alert: any) {
  return (alert.labels || {}).severity || 'info'
}

function setFilter(severity: string | null) {
  if (filterSeverity.value === severity) {
...
function severityLabel(severity: string) {
  return severityStatusText(severity)
}
...
async function loadAlerts() {
  ...
  errorMsg.value = ''
  try {
    const res = await getCIPrometheusAlerts(props.ciId)
    configured.value = res.configured !== false
    alerts.value = res.alerts || []
    displayColumns.value = res.display_columns || []
    connectionStatuses.value = res.connection_status || []
    ...
    loading.value = false
    emit('connectionStatusChange', connectionStatuses.value)
  }
}
...
function severityStatus(severity: string): 'error' | 'warning' | 'processing' {
...
function severityStatusText(severity: string) {
  const s = (severity || '').toLowerCase()
  if (s === 'disaster') return t('cmdb.ci.alertDisaster')
  if (s === 'emergency') return t('cmdb.ci.alertEmergency')
  if (s === 'critical') return t('cmdb.ci.alertCritical')
  if (s === 'important') return t('cmdb.ci.alertImportant')
  if (s === 'warning') return t('cmdb.ci.alertWarning')
  return t('cmdb.ci.alertInfo')
}
...
function formatDuration(activeAt: any) {
  if (!activeAt) return '-'
  ...
  const isZh = locale.value === 'zh'
  const parts: string[] = []
  if (days > 0) parts.push(days + (isZh ? t('cmdb.ci.alertDurationDay') : ' ' + t('cmdb.ci.alertDurationDay') + ' '))
  if (hours > 0) parts.push(hours + (isZh ? t('cmdb.ci.alertDurationHour') : ' ' + t('cmdb.ci.alertDurationHour') + ' '))
  if (minutes > 0) parts.push(minutes + (isZh ? t('cmdb.ci.alertDurationMinute') : ' ' + t('cmdb.ci.alertDurationMinute') + ' '))
  if (seconds > 0 || parts.length === 0) parts.push(seconds + (isZh ? t('cmdb.ci.alertDurationSecond') : ' ' + t('cmdb.ci.alertDurationSecond')))
  ...
  <div class="ci-detail-prometheus">
      ...
      <template #description>
        <div class="prom-alert-desc">
          ...
          <a-button size="small" type="link" @click="loadAlerts" :loading="loading">
            <ReloadOutlined />{{ t('cmdb.ci.prometheusConnectionRetry') }}
          </a-button>
        </div>
      </template>
    </a-alert>
    ...
    <div v-else class="prom-normal-content">
      ...
      <div class="prom-alert-stats">
        ...
        >
          <div class="stat-icon"><component :is="sev.icon" /></div>
          <div class="stat-content">
        ...
        <div class="prom-refresh-area">
          ...
          <a-button size="small" @click="loadAlerts" :loading="loading">
            <ReloadOutlined />{{ t('cmdb.ci.alertRefresh') }}
          </a-button>
        </div>
      </div>
      ...
      <div v-if="filterSeverity !== null" class="prom-filter-bar">
        ...
        <a-button size="small" type="link" @click="setFilter(null)">
          <CloseCircleOutlined />{{ t('cmdb.ci.alertClearFilter') }}
        </a-button>
      </div>
        ...
        <a-spin :spinning="loading" class="prom-alert-spin">
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'activeAt'">
                {{ formatTime(record.activeAt) }}
              </template>
