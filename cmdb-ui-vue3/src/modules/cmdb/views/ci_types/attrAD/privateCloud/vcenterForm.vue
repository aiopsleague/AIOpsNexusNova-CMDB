...
async function init(id: string | number) {
  ...
  nextTick(() => {
    const { _reference = '', host = '', account = '', tabActive } = props.value || {}
...
function handleSelectChange(id: any) {
  ...
  const { host, account } = props.value
  emit('change', {
    ...props.value,
...
<template>
  <div class="private-cloud-wrap">
    ...
    >
      ...
      <a-form-item :required="true" :label="t('cmdb.ciType.host')">
        <a-input
          v-model:value="formData.host"
          :disabled="formData.tabActive === TAB_KEY.CONFIG"
        />
      </a-form-item>
      <a-form-item :required="true" :label="t('cmdb.ciType.account')">
        <a-input
          v-model:value="formData.account"
          :disabled="formData.tabActive === TAB_KEY.CONFIG"
        />
      </a-form-item>
      <a-form-item v-if="formData.tabActive === TAB_KEY.CUSTOM" :required="true" :label="t('cmdb.ciType.password')">
        <a-input-password v-model:value="formData.password" />
      </a-form-item>
      <a-form-item :label="t('cmdb.ciType.vcenterName')">
        <a-input v-model:value="formData.vcenterName" />
      </a-form-item>
    </a-form>
  </div>
</template>
