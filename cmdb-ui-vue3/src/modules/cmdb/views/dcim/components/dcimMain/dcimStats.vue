...
import { DatabaseOutlined, DesktopOutlined } from '@ant-design/icons-vue'
...
const statsList = computed<StatsItem[]>(() => {
  const { device_count = 0, rack_count = 0, u_count = 0, u_used_count = 0 } = props.statsData || {}
...
<template>
  <div class="dcim-stats">
    <div v-for="(item, index) in statsList" :key="index" class="dcim-stats-card">
      <div class="dcim-stats-card-left">
        <div class="dcim-stat-card-title">{{ t(item.title) }}</div>

        <div class="dcim-stats-card-row">
          <div v-for="(data, dataIndex) in item.countList" :key="dataIndex" class="dcim-stats-card-count">
      ...
      <div v-if="item.icon" class="dcim-stats-card-icon">
        <component :is="item.icon" />
      </div>

      <DCIMStatsChart v-else-if="item.chartData" :chart-data="item.chartData" :chart-ratio="item.chartRatio" />
    </div>
  </div>
</template>
...
.dcim-stats {
  ...
  &-card {
    ...
    filter: drop-shadow(0px 0px 12px rgba(231, 236, 239, 0.1));
