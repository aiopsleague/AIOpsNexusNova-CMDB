...
export const category_1_bar_options = (data: any, options: any) => {
  // Calculate first level classification
  const xData = Object.keys(data)
  // Calculate how many secondary categories there are
  const secondCategory: Record<string, number[]> = {}
  Object.keys(data).forEach((key) => {
    if (Object.prototype.toString.call(data[key]) === '[object Object]') {
      Object.keys(data[key]).forEach((key1) => {
        secondCategory[key1] = Array.from({ length: xData.length }).fill(0) as number[]
      })
    } else {
      secondCategory[i18n.global.t('cmdb.common.other')] = Array.from({ length: xData.length }).fill(0) as number[]
    }
  })
  Object.keys(secondCategory).forEach((key) => {
    xData.forEach((x, idx) => {
      ...
      if (typeof data[x] === 'number') {
        secondCategory[i18n.global.t('cmdb.common.other')][idx] = data[x]
      }
    })
  })
  return {
    color: (options?.chartColor ?? DEFAULT_COLORS).split(','),
    grid: {
      top: 15,
      left: 'left',
      right: 10,
...
export const category_1_line_options = (data: any, options: any) => {
  const xData = Object.keys(data)
  return {
    color: (options?.chartColor ?? DEFAULT_COLORS).split(','),
    grid: {
          ...
          ? {
              ...
              color: {
                ...
                colorStops: [
                  {
                    offset: 0,
                    color: (options?.chartColor ?? DEFAULT_COLORS).split(',')[0],
                  },
                  {
                    offset: 1,
                    color: '#ffffff',
...
export const category_1_pie_options = (data: any, options: any) => {
  return {
    color: (options?.chartColor ?? DEFAULT_COLORS).split(','),
    grid: {
      top: 10,
...
export const category_2_bar_options = (data: any, options: any, chartType: string) => {
  ...
  return {
    ...
    xAxis:
      options.barDirection === 'y' || chartType === 'line'
        ? {
            type: 'category',
            axisTick: { show: false },
            data: xAxisData,
    ...
    series: legend.map((le, index) => {
      return {
        ...
        barMaxWidth: '16px',
        areaStyle:
          chartType === 'line' && options?.isShadow
            ? {
                opacity: 0.5,
                color: {
                  type: 'linear',
                  ...
                  colorStops: [
                    {
                      offset: 0,
                      color: (options?.chartColor ?? DEFAULT_COLORS).split(',')[index % 8],
                    },
                    {
                      offset: 1,
                      color: '#ffffff',
...
export const category_2_pie_options = (data: any, options: any) => {
  const _legend: Array<{ value: number; name: string }> = []
