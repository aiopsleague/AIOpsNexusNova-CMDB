...
watch([cycle01, cycle02], ([c1, c2]) => {
  cycle01.value = props.check(c1, 0, 23)
  cycle02.value = props.check(c2, 0, 23)
  if (radioValue.value === 2) emit('update', 'hour', cycleTotal.value)
})
watch([average01, average02], ([a1, a2]) => {
  average01.value = props.check(a1, 0, 23)
  average02.value = props.check(a2, 1, 23)
...
<template>
  <a-form size="small">
    <a-radio-group v-model:value="radioValue" style="width: 100%">
      <a-form-item>
        <a-radio :value="1">小时，允许的通配符[, - * /]</a-radio>
      </a-form-item>
      <a-form-item>
        <a-radio :value="2">
          周期从
          <a-input-number v-model:value="cycle01" :min="0" :max="23" /> -
          <a-input-number v-model:value="cycle02" :min="0" :max="23" /> 小时
        </a-radio>
      </a-form-item>
      <a-form-item>
        <a-radio :value="3">
          从
          <a-input-number v-model:value="average01" :min="0" :max="23" /> 小时开始，每
          <a-input-number v-model:value="average02" :min="0" :max="23" /> 小时执行一次
        </a-radio>
      </a-form-item>
      <a-form-item>
        <a-radio :value="4">
