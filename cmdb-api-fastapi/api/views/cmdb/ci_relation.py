...
@router.get("/ci_relations/s")
@router.get("/ci_relations/search")
def ci_relation_search_view_get():
...
@router.post("/ci_relations/path/s")
@router.post("/ci_relations/path/search")
@args_required("source", "target", "path")
def ci_relation_search_path_view_post():
...
@router.get("/ci_relations/statistics")
def ci_relation_statistics_view_get():
...
@router.get("/ci_relations/search/full")
def ci_relation_search_full_view_get():
    root_ids = list(map(int, handle_arg_list(request.values.get('root_ids'))))
    level = request.values.get('level', 1)
...
@router.get("/ci_relations/{first_ci_id:int}/second_cis")
def get_second_cis_view_get(first_ci_id: int):
    page = get_page(request.values.get("page", 1))
...
@router.get("/ci_relations/{second_ci_id:int}/first_cis")
def get_first_cis_view_get(second_ci_id: int):
    page = get_page(request.values.get("page", 1))
    count = get_page_size(request.values.get("count"))
...
@router.post("/ci_relations/{first_ci_id:int}/{second_ci_id:int}")
def ci_relation_view_post(first_ci_id: int, second_ci_id: int):
    ancestor_ids = request.values.get('ancestor_ids') or None

    manager = CIRelationManager()
    res = manager.add(first_ci_id, second_ci_id, ancestor_ids=ancestor_ids)

    return dict(cr_id=res)


@router.delete("/ci_relations/{first_ci_id:int}/{second_ci_id:int}")
def ci_relation_view_delete(first_ci_id: int, second_ci_id: int):
    ancestor_ids = request.values.get('ancestor_ids') or None

    manager = CIRelationManager()
    manager.delete_2(first_ci_id, second_ci_id, ancestor_ids=ancestor_ids)

    return dict(message="CIType Relation is deleted")


@router.delete("/ci_relations/{cr_id:int}")
def delete_ci_relation_view_delete(cr_id: int):
    manager = CIRelationManager()
...
@router.post("/ci_relations/batch")
@args_required('ci_ids')
def batch_create_or_update_ci_relation_view_post():
...
@router.put("/ci_relations/batch")
@args_required('ci_ids')
@args_required('parents')
def batch_create_or_update_ci_relation_view_put():
    return batch_create_or_update_ci_relation_view_post()


@router.delete("/ci_relations/batch")
@args_required('ci_ids')
@args_required('parents')
def batch_create_or_update_ci_relation_view_delete():
    ci_ids = list(map(int, request.values.get('ci_ids')))
    parents = list(map(int, request.values.get('parents', [])))
    ancestor_ids = request.values.get('ancestor_ids') or None
