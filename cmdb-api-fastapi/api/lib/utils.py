    ...
    def init_app(self, app=None):
        ...
        try:
            pool = redis.ConnectionPool(
                max_connections=getattr(config, "REDIS_MAX_CONN", None),
                host=getattr(config, "CACHE_REDIS_HOST"),
                port=getattr(config, "CACHE_REDIS_PORT"),
                password=getattr(config, "CACHE_REDIS_PASSWORD") or None,
                db=getattr(config, "REDIS_DB", None) or 0)
...
class ESHandler(object):
    ...
    def init_app(self, app=None):
        import settings
        config = settings
        if getattr(config, 'ES_USER', None) and getattr(config, 'ES_PASSWORD', None):
            uri = "http://{}:{}@{}:{}/".format(config.ES_USER, config.ES_PASSWORD,
                                               getattr(config, 'ES_HOST'), getattr(config, 'ES_PORT', 9200))
        else:
            uri = "{}:{}".format(getattr(config, 'ES_HOST'), getattr(config, 'ES_PORT', None) or 9200)
