...
def auth_required(func):
    # NOTE(fastapi-port): request.values normalization now happens in
    # RequestContextMiddleware; the auth dispatch below is also exposed as
    # the FastAPI dependency ``authenticate`` (used for function endpoints).
    @wraps(func)
...
def authenticate(starlette_request: StarletteRequest):
    """FastAPI dependency equivalent of the ``auth_required`` wrapper.

    Reads the auth flags (``authenticated`` / ``auth_with_app_token`` /
    ``auth_only_with_app_token``) from the endpoint function itself, which
    keeps decorators like ``@auth_abandoned`` working unchanged.
    """
    func = starlette_request.scope.get("endpoint")
